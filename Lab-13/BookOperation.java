...
public class BookOperation extends BookManagement1 // extend the functionality of Book_Mnagenet class to Book_operation class
{
	...
	public void add_book() // function to add book details using throws exception passing the Connection conn object for connect with the database.
	{
		try {
			...
			do {
				...
				String qur = "insert into book_details (book_id,book_name,book_price,book_qunt) value(?,?,?,?)";// insert sql query for insert data into the  book_details table.
	...
	public void edit_book() // function to edit  book details using throws exception passing the Connection conn object for connect with the database.
	{
			...
			int ch1 =sc.nextInt();// accept choice form user to which field user want to update
				...
				switch(ch1) {
					case 1:
						try {
							...
							PreparedStatement ps = conn.prepareStatement("UPDATE book_details SET book_name = ? WHERE book_id = ?");// update sql query for update data into the table
				...
				case 2:
					try {
						...
						PreparedStatement ps1 = conn.prepareStatement("UPDATE book_details SET book_price = ? WHERE book_id = ?");// update sql query for update data into the table
				...
				case 3:
					try {
						...
						PreparedStatement ps2 = conn.prepareStatement("UPDATE book_details SET book_qunt = ? WHERE book_id = ?");// update sql query for update data into the table
						System.out.println("Enter The New Quantity Of The Book");
						ps2.setInt(1, sc.nextInt());// set the new quantity for the book
	...
	public void delete_book() // function to delete book details using throws exception passing the Connection conn object for connect with the database.
	{
		try {
			Connection conn = setConnection();
			...
			do {
				...
				PreparedStatement ps = conn.prepareStatement("delete from book_details WHERE book_id=?");// create sql statement for delete book data from book_details table.
	...
	public void get_book_byId()// function to get book details by using book_id using throws exception passing the Connection conn object for connect with the database.
	{
		try {
			...
			do {
				String qur = "select *from book_details where book_id=?";// sql query for retrive data from table
				System.out.println("Enter Book Id : ");
	...
	public void all_books_details() // function to get all book details  using throws exception passing the Connection conn object for connect with the database.
	{
		try {
			Connection conn = setConnection();

			PreparedStatement ps = conn.prepareStatement("Select *from book_details");// create sql statement for display all books data form table to java application.
	...
	public void assign_author()// function to assign author to a book using throws exception passing the Connection conn object for connect with the database.
	{
		try {
			Connection conn=setConnection();
